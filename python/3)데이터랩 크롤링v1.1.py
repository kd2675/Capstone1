...
class NaverDataLabOpenAPI():
    ...
    def get_data(self, startDate, endDate, timeUnit, device, ages, gender):
        ...
        if(rescode==200):
            ...
            ratio_result = ratio_result2 - ratio_result1
            
            jsonResult.append({'title':tag, 'result':ratio_result})
            self.df = df
...
jsonResult =[]
inputFileName = '2)실시간_search_news'
...
for tag in message_N:
    ...
    if(len(str(tag))>1):
        ...
        jsonResult.sort(reverse=True, key=lambda x: (x['result'], x['title']))
with open('3)%s_per_%s.json' % ('실시간', 'news'), 'w', encoding='utf8') as outfile:
